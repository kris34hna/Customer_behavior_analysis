...
Data Extraction (SQL) — Wrote 10 analytical queries covering revenue by gender, discount behavior, product ratings, shipping comparison, subscriber vs. non-subscriber spend, customer segmentation (New / Returning / Loyal), top products by category, repeat buyer behavior, and age-group revenue contribution.
Exploratory Data Analysis (Python) — Cleaned the dataset, handled missing values, created the Age_Group derived column, and produced visualizations for distributions, correlations, and trends using Pandas, Matplotlib, and Seaborn.
Dashboard (Power BI) — Built a 3-page interactive dashboard with KPI cards, bar charts, donut charts, and category filters to make insights accessible to business stakeholders.
